...
neig.shp <- rgdal::readOGR(dsn=getwd(), stringsAsFactors=TRUE,
                            layer = "TXNeighbors", integer64 = "allow.loss")

## Get polygons of TX counties
county.shp <- rgdal::readOGR(dsn=getwd(), stringsAsFactors=TRUE,
                           layer = "TXCnty", integer64 = "allow.loss")

## Add a seqId to the record names to identify each record
row.names(county.shp@data) <- 1:length(county.shp)            

## Get interstate layer
interState.shp <- rgdal::readOGR(dsn=getwd(), stringsAsFactors=TRUE,
...
## Example: dropping specific counties simultaneously from the map and attribute table
##          Both the map and the attribute table must remain synchronized.
cntyShort.shp <- county.shp[-c(8),]   # drop Loving Cnty
...
mapColorQual(cntyShort.shp$URBRURAL, cntyShort.shp,
