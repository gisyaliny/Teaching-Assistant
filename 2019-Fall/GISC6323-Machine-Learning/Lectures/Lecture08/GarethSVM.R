...
## Change cost value
svmfit <- svm(y~., data=df, kernel="linear", cost=.01, scale=FALSE)
ypred <- predict(svmfit,testdat)
table(predict=ypred, truth=testdat$y)

## Define linearily sepapable training sample
x[y==1,] <- x[y==1,]+0.5
plot(x, col=(y+5)/2, cex=1.5, pch=19)
df <- data.frame(x=x, y=as.factor(y))
...
set.seed(1)
tune.out <- tune(svm, y~., data=df[train,], kernel="radial", ranges=list(cost=c(0.1,1,10,100,1000),gamma=c(0.5,1,2,3,4)))
summary(tune.out)
summary(tune.out$best.model)
table(true=dat[-train,"y"], pred=predict(tune.out$best.model,newdata=df[-train,]))
