...
## Find observations with highest uncertainty
extremObs <- head(sort(p.Mclust$uncertainty, decreasing=TRUE), n=6)
for (i in 1:length(extremObs)) print(which(p.Mclust$uncertainty == extremObs[i]))
